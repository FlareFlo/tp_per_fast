...
impl BinarySearch {
	...
	fn from_square(square: Rect, i: usize, bezirke: &[Bezirk], limit: usize, chunks: &mut Vec<(Rect, Vec<Bezirk>)>) {
		...
		if i >= limit {
			let in_chunk = bezirke.iter().filter(|e|square.intersects(&e.location)).cloned().collect();
	...
	pub fn lookup(&self, point: Point) -> Option<&Bezirk> {
		let valid_chunks = self.chunks.iter().filter(|(rect, chunks)|rect.contains(&point)).map(|e|&e.1);
